...
// In-memory KB: { id, url, lang:'en', chunk, vec?: number[] }
let KB = [];
// Sparse index:
let VOCAB_IDF = new Map();   // term -> idf
let CHUNK_TF = new Map();    // chunkId -> Map(term -> tf)
  ...
  try{
    ...
    if(r.ok){
      ...
      while((m=re.exec(xml))){
        ...
        // only EN root paths (skip /fi and /sv)
        const path = new URL(u).pathname.replace(/^\/+/,"");
        if(path.startsWith("fi")||path==="fi"||path.startsWith("sv")||path==="sv") continue;
...
async function crawl(){
  ...
  for(const d of KB){
    ...
    // TF by whole-word counts (fallback 1)
    const tf = new Map();
  ...
  for(const d of KB){
    ...
    if(tf){
      for(const t of terms){
        ...
        const denom = 1 + 0.25 + f; // BM25-ish simplified
...
$form.addEventListener("submit",async e=>{
  ...
  try{
    const r=await fetch("/chat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({message:text,lang:$lang.value})});
